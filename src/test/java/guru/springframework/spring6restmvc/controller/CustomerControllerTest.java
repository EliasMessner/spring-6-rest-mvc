...
class CustomerControllerTest {
    ...
    void patchCustomer() throws Exception {
        ...
        Map<String, Object> valuesToPatch = new HashMap<>();
        valuesToPatch.put("name", "New Customer");

        mockMvc.perform(patch("/api/v1/customer/" + customer.getId())
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(valuesToPatch)))
                .andExpect(status().isNoContent());

        verify(customerService).patchById(uuidArgumentCaptor.capture(), customerArgumentCaptor.capture());
        assertThat(uuidArgumentCaptor.getValue()).isEqualTo(customer.getId());
        assertThat(customerArgumentCaptor.getValue().getName()).isEqualTo(valuesToPatch.get("name"));
    }
